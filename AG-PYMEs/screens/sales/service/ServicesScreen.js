...
const ServicesScreen = () => {
  ...
  useEffect(() => {
    filterServices();
  }, [services, searchQuery, activeCategory, activeFilter]);
  ...
  const updateServiceStatus = async (serviceId, newStatus) => {
    ...
    try {
      ...
      const serviceToUpdate = services.find((s) => s.id_servicio === serviceId);
      if (!serviceToUpdate) {
      ...
      >
        <View style={styles.filtersContainer}>
          <View style={styles.categoriesContainer}>
            <Text
              style={[styles.filtersTitle, { color: themeObject.colors.text }]}
            >
              Categorías:
            </Text>
            <ScrollView
