...
const useEmployeeManagement = () => {
  ...
  useEffect(() => {
    const loadLeaves = async () => {
      ...
      try {
        ...
        setEmployeeLeaves(
          leaves.filter((l) => l.id_empleado === selectedEmployee.id_empleado)
        );
      } catch (error) {
  ...
  const handleSelectEmployee = useCallback((employee) => {
    setSelectedEmployee((prevSelected) => {
      if (prevSelected?.id_empleado === employee.id_empleado) {
        setIsEditing(false);
        return null;
      }
      setIsEditing(true);
  ...
  }, [employees, employeeForm, selectedEmployee]);
  ...
  }, [employees, employeeLeaves, leaveForm, selectedEmployee]);

  const handleReactivateEmployee = useCallback(async () => {
    if (!selectedEmployee) return;
  ...
  const handleDeleteEmployee = useCallback(async () => {
    ...
    try {
      ...
      setEmployees(
        employees.filter((e) => e.id_empleado !== selectedEmployee.id_empleado)
      );
