...
const EmployeesBody = forwardRef(
  (
    { onEmployeeUpdate, hideSearchBar = false, externalSearchQuery = "" },
    ref
  ) => {
    ...
    const loadEmployees = async () => {
      try {
        ...
        showError(
          "Error",
          `No se pudieron cargar los empleados: ${err.message}`
        );
      } finally {
        setLoading(false);
      }
      ...
      showConfirmDialog(
        ...
        async () => {
          try {
            ...
            // Actualizar estado local
            setEmployees((prev) =>
              prev.filter((emp) => emp.id_empleado !== id)
            );
