...
async function getAllClients(companyId, filters = {}) {
  try {
...
async function createClient(companyId, clientData) {
  try {
    ...
    const { data: existingClient } = await supabaseAdmin
      ...
      .select("id_cliente")
      .eq("company_id", companyId)
      .eq("documento", clientData.documento.toUpperCase().replace(/\s/g, ""))
      .single();
...
async function updateClient(companyId, clientId, clientData) {
  try {
    ...
    if (clientData.nombre !== undefined) updateData.nombre = clientData.nombre;
    ...
    if (clientData.ciudad !== undefined) updateData.ciudad = clientData.ciudad;
...
async function deleteClient(companyId, clientId) {
  try {
    ...
    // Verificar si tiene ventas asociadas (si existe tabla de ventas)
    // Esta verificación se haría con la tabla correspondiente cuando exista
...
async function getClientStats(companyId) {
  try {
    ...
    if (clientsError) {
      console.error("Error al obtener estadísticas de clientes:", clientsError);
      throw clientsError;
    }
