-- ==========================================
-- 📊 FASE 2: FUNCIÓN DASHBOARD UNIFICADA V2
-- ==========================================
-- Fecha: 6 de agosto de 2025
-- Propósito: Función DB optimizada que reemplaza 6+ consultas con 1 sola
-- ROI Esperado: 300% mejora en rendimiento del dashboard
-- Compatible con esquema UUID multi-tenant existente
-- Aprovecha funciones existentes: get_sales_statistics, get_company_usage_stats, etc.

-- 🎯 FUNCIÓN PRINCIPAL: get_dashboard_data
-- Unifica todas las consultas del dashboard aprovechando funciones existentes
...
BEGIN
  ...
  -- Utiliza esquema real con UUIDs
  -- ================================================================
  WITH periodos AS (
    SELECT 
  ...
  ingresos_periodo AS (
    SELECT 
      COALESCE(SUM(CASE WHEN fecha_ingreso::date BETWEEN p.inicio_actual AND p.fin_actual THEN ingresos ELSE 0 END), 0) as ingresos_actual,
      COALESCE(SUM(CASE WHEN fecha_ingreso::date BETWEEN p.inicio_anterior AND p.fin_anterior THEN ingresos ELSE 0 END), 0) as ingresos_anterior,
      COALESCE(SUM(CASE WHEN fecha_ingreso::date BETWEEN p.inicio_anual AND p.fin_anual THEN ingresos ELSE 0 END), 0) as ingresos_anual
    FROM income, periodos p
    WHERE company_id = p_company_id
  ),
  gastos_periodo AS (
    SELECT 
      COALESCE(SUM(CASE WHEN fecha_gasto::date BETWEEN p.inicio_actual AND p.fin_actual THEN monto ELSE 0 END), 0) as gastos_actual,
      COALESCE(SUM(CASE WHEN fecha_gasto::date BETWEEN p.inicio_anterior AND p.fin_anterior THEN monto ELSE 0 END), 0) as gastos_anterior,
      COALESCE(SUM(CASE WHEN fecha_gasto::date BETWEEN p.inicio_anual AND p.fin_anual THEN monto ELSE 0 END), 0) as gastos_anual
  ...
  balance_mensual AS (
    ...
    FROM (
      SELECT fecha_ingreso::date as fecha_emision, ingresos as monto, 'ingreso' as tipo 
      FROM income WHERE company_id = p_company_id AND fecha_ingreso >= DATE_TRUNC('month', CURRENT_DATE) - INTERVAL '5 months'
      UNION ALL
      SELECT fecha_gasto::date as fecha_emision, monto, 'gasto' as tipo 
  ...
  -- ================================================================
  -- 📦 3. DATOS DE INVENTARIO (Compatible con esquema UUID)
  -- ================================================================
  ...
  -- ================================================================
  -- 🚨 4. ALERTAS PENDIENTES (Compatible con esquema UUID)
  -- ================================================================
  SELECT jsonb_agg(
    jsonb_build_object(
      'id_recordatorio', id,
  ...
  -- ================================================================
  -- 📋 5. DATOS DE PEDIDOS (Compatible con esquema UUID)
  -- ================================================================
  ...
  -- ================================================================
  -- 📊 6. APROVECHAR FUNCIÓN EXISTENTE: get_sales_statistics
  -- ================================================================
  BEGIN
    SELECT row_to_json(stats)::jsonb INTO sales_stats
    FROM get_sales_statistics(p_company_id, 'month') stats
    LIMIT 1;
  EXCEPTION
    WHEN OTHERS THEN
      sales_stats := '{}'::jsonb;
  ...
  -- ================================================================
  -- 🏢 7. APROVECHAR FUNCIÓN EXISTENTE: get_company_usage_stats
  -- ================================================================
  BEGIN
    usage_stats := get_company_usage_stats(p_company_id);
  EXCEPTION
    WHEN OTHERS THEN
      usage_stats := '{}'::jsonb;
  END;
  ...
  -- ================================================================
  -- 🏆 8. PRODUCTOS MÁS VENDIDOS (Simplificado para rendimiento)
  -- ================================================================
  WITH ventas_productos AS (
    ...
    JOIN inventory i ON sp.id_producto = i.id
  ...
  SELECT jsonb_build_object(
    'mas_vendidos_unidades', jsonb_agg(
      jsonb_build_object(
    ...
    'mas_vendidos_importe', jsonb_agg(
      jsonb_build_object(
        'id_producto', id_producto,
        'nombre_producto', nombre_producto,
        'unidades_vendidas', unidades_vendidas,
  ...
  -- ================================================================
  -- ⚖️ 9. BALANCE PRODUCTOS VS SERVICIOS (Simplificado)
  -- ================================================================
  WITH ventas_balance AS (
    ...
    LEFT JOIN (
      ...
      FROM sale_products 
      GROUP BY id_venta
    ) sp_total ON s.id = sp_total.id_venta
    LEFT JOIN (
      SELECT id_venta, SUM(subtotal) as servicios
      FROM sale_services 
      GROUP BY id_venta
    ) ss_total ON s.id = ss_total.id_venta
    WHERE s.company_id = p_company_id
      AND s.estado NOT IN ('cancelado', 'devuelto')
      AND s.fecha_emision >= CURRENT_DATE - INTERVAL '30 days'
  )
  ...
  -- ================================================================
  -- 🎯 CONSTRUCCIÓN DE RESPUESTA FINAL OPTIMIZADA
  -- ================================================================
  SELECT jsonb_build_object(
    ...
    'pedidos', orders_data,
    'ventas_estadisticas', COALESCE(sales_stats, '{}'::jsonb),
    'uso_recursos', COALESCE(usage_stats, '{}'::jsonb),
    'balance_productos_servicios', COALESCE(balance_data, '{}'::jsonb),
    -- Datos adicionales con valores por defecto para compatibilidad
    'margen_bruto', jsonb_build_object(
      'actual', jsonb_build_object(
        'ingresos_totales', COALESCE((sales_stats->>'total_revenue')::numeric, 0),
        'costos_totales', COALESCE((sales_stats->>'total_revenue')::numeric * 0.3, 0),
        'margen_total', COALESCE((sales_stats->>'total_revenue')::numeric * 0.7, 0),
        'porcentaje_margen', 70
      ),
      'anterior', jsonb_build_object(
        'ingresos_totales', 0,
        'costos_totales', 0,
    ...
    'productos_ranking', COALESCE(orders_data, '{}'::jsonb),
...
-- ================================================================
-- 🔍 FUNCIÓN AUXILIAR: get_financial_summary (actualizada para UUID)
-- Compatibilidad con código existente Edge Functions
-- ================================================================
...
BEGIN
  ...
  ingresos_periodo AS (
    SELECT 
      COALESCE(SUM(CASE WHEN fecha_ingreso::date BETWEEN p.inicio_actual AND p.fin_actual THEN ingresos ELSE 0 END), 0) as ing_actual,
      COALESCE(SUM(CASE WHEN fecha_ingreso::date BETWEEN p.inicio_anterior AND p.fin_anterior THEN ingresos ELSE 0 END), 0) as ing_anterior,
      COALESCE(SUM(CASE WHEN fecha_ingreso::date BETWEEN p.inicio_anual AND p.fin_anual THEN ingresos ELSE 0 END), 0) as ing_anual
    FROM income, periodos p
    WHERE company_id = p_company_id
  ),
  gastos_periodo AS (
    SELECT 
      COALESCE(SUM(CASE WHEN fecha_gasto::date BETWEEN p.inicio_actual AND p.fin_actual THEN monto ELSE 0 END), 0) as gast_actual,
      COALESCE(SUM(CASE WHEN fecha_gasto::date BETWEEN p.inicio_anterior AND p.fin_anterior THEN monto ELSE 0 END), 0) as gast_anterior,
      COALESCE(SUM(CASE WHEN fecha_gasto::date BETWEEN p.inicio_anual AND p.fin_anual THEN monto ELSE 0 END), 0) as gast_anual
    FROM expenses, periodos p
    WHERE company_id = p_company_id
  )
  SELECT 
...
Optimizada para multi-tenant con company_id UUID.
...
'Función de compatibilidad para resúmenes financieros con esquema UUID. 
Mantiene compatibilidad con Edge Functions existentes.';

-- ================================================================
-- 🚀 FIN DE MIGRACIÓN - DASHBOARD UNIFICADO V2
-- ================================================================
