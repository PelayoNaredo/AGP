 ...
 * 
 * @param {boolean} enabled - Estado actual del switch (activado/desactivado)
 * @param {function} onToggle - Función para cambiar el estado del switch
 * @param {object} supplier - Objeto con los datos del proveedor seleccionado
 * @returns {JSX.Element} - Componente de UI
 */
  ...
  return (
    <View style={styles.switchContainer}>
      ...
      <Switch
        ...
        thumbColor={Platform.OS === 'android' ? themeObject.colors.white : undefined}
      />
    </View>
  );
};
...
export const processEmailTemplate = (template, order, items, supplier) => {
  ...
  const replacements = {
    ...
    '{NOMBRE_EMPRESA}': 'Mi Empresa', // Esto debería venir de la configuración
...
export const sendEmail = async (to, subject, body) => {
  try {
    ...
    if (Platform.OS === 'web') {
      ...
      if (!opened) {
        console.warn('No se pudo abrir Gmail automáticamente (posible bloqueador de ventanas emergentes)');
        ...
        alert('Si el correo no se abre automáticamente, por favor habilite las ventanas emergentes para este sitio o utilice el enlace que apareció en pantalla.');
