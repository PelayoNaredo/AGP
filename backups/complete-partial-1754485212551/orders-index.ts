...
function validateOrderData(data: any, isUpdate = false) {
  const errors: string[] = [];
...
async function handleSecureRoute(req: Request) {
  ...
  if (req.method === "GET") {
    ...
    if (pathSegments.length === 0 && searchParams.has("status")) {
      ...
      if (!estadosValidos.includes(status.toLowerCase())) {
        return new Response(
          JSON.stringify({
    ...
    if (pathSegments.length === 0 && searchParams.has("supplier")) {
      ...
      try {
        ...
        if (!existingSupplier) {
          return createCorsErrorResponse("Proveedor no encontrado", 404);
        }
  ...
  if (req.method === "POST") {
    ...
    if (pathSegments.length === 0) {
      try {
        ...
        if (!existingSupplier) {
          return createCorsErrorResponse("El proveedor especificado no existe", 400);
        }
